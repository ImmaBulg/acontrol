...
class FormMeter extends Model
{
	...
	public function rules()
	{
		return [
			...
			['data_usage_method', 'in', 'range' => array_keys(Meter::getListDataUsageMethods()), 'skipOnEmpty' => false],
	...
	public function attributeLabels()
	{
		return [
			...
			'expire_date' => Yii::t('backend.meter', 'Expire date'),
		];
	}
	...
	public function edit()
	{
		...
		try	{
			...
			$model->site_id = $this->site_id;
			$model->ip_address = $this->ip_address;
			$model->expire_date = $this->expire_date;
			...
			if ($updated_type) {
				...
				MeterRawData::deleteAll('meter_id = :name', ['name' => $model->name]);
				MeterRawData::deleteCacheValue(["meter_raw_data:{$model->name}"]);
