...
class FormAirMeterRawData extends FormMeterRawData
{
    ...
    protected function generateDataProvider() {
        ...
        if($direction) {
            for($i = $from_date; $i < $to_date; $i = $i + FormMeterRawDataSingle::_1_HOUR) {
                $this->calculateData($i, $rows, $apply_avg, $avg_data, $avg_readings, $data, $sql_date_format, $avg_readings);
            }
        }
        else {
            for($i = $to_date; $i >= $from_date; $i = $i - FormMeterRawDataSingle::_1_HOUR) {
    ...
    public function calculateData($i, $rows, $apply_avg, $avg_data, &$avg_readings, &$data, $sql_date_format, $direction = 1) {
