 ...
 */
class FormTenant extends \yii\base\Model
{
	...
	public function rules()
	{
		return [
			...
			['exit_date', '\common\components\validators\DateTimeCompareValidator', 'compareAttribute' => 'entrance_date', 'format' => Formatter::PHP_DATE_FORMAT, 'operator' => '>='],
			...
			['included_reports', 'each', 'rule' => ['in', 'range' => array_keys(Report::getListTypes()), 'skipOnEmpty' => true]],
            ...
            [['formatting'], 'match', 'pattern' => '/^(@(prefix|property|contract|code|date|total|ending){1})+$/', 'message' => 'Error pattern!'],
		...
		foreach ($rule_single_channels as $rule_single_channel) {
			...
			if ($result != null) {
				...
				if ($sum > RuleSingleChannel::MAX_PERCENT) {
					return $this->addError($attribute, Yii::t('backend.rule', 'This tenant has conflict rules with next tenants: {tenants}.', [
