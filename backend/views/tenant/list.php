...
$this->title = Yii::t('backend.view', 'Tenants');
				...
				Dropdown::widget([
					'items' => [
						[
							...
							'url' => ArrayHelper::merge(Yii::$app->request->get(), ['/tenant/create']),
						],
					],
				]); ?>
			</div>
		...
		'columns' => [
			...
			[
				...
				'value' => function ($model){
					...
					$btn[] = '<div class="btn-group">' .Html::a(Yii::t('backend.view', 'Contacts ({count})', ['count' => $model->getRelationTenantContacts()->andWhere(['status' => TenantContact::STATUS_ACTIVE])->count()]), ['/tenant-contact/list', 'id' => $model->id], ['class' => 'btn btn-lilac btn-sm']). '</div>';
