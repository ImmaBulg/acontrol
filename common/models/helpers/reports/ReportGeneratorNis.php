    ...
    public function calculateFixedRules(RuleFixedLoad $rule, Tenant $tenant)
    {
        ...
        switch($rule['use_type'])
        {
            case RuleFixedLoad::USE_TYPE_FLAT_ADDITION_TOTAL_BILL_AMOUNT:
                ...
                $this->data['tenants'][$tenant->id]['total'] += ($this->data['tenants'][$tenant->id]['total']['total_pay'] * ((int)$rule['value'] / 100));
                $this->data['site_total'] += ($this->data['tenants'][$tenant->id]['total']['total_pay'] * ((int)$rule['value'] / 100));
                break;
            case RuleFixedLoad::USE_TYPE_MONEY:
                ...
                $this->data['tenants'][$tenant->id]['total'] += (int)$rule['value'];
                $this->data['site_total'] += (int)$rule['value'];
                break;
            case RuleFixedLoad::USE_TYPE_KWH_FIXED:
                ...
                $this->data['tenants'][$tenant->id]['total'] += ((int)$this->data['tenants'][$tenant->id]['total_fixed_rules']);
            ...
            case RuleFixedLoad::USE_TYPE_FLAT_ADDITION_TOTAL_USAGE:
                ...
                $this->data['tenants'][$tenant->id]['total_pay'] += (int)$this->data['tenants'][$tenant->id]['total_fixed_rules'];
                $this->data['tenants'][$tenant->id]['total'] += (int)$this->data['tenants'][$tenant->id]['total_fixed_rules'];
                $this->data['site_total'] += (int)$this->data['tenants'][$tenant->id]['total_fixed_rules'];
                break;
        }
    }
